...
namespace WeaponShopV20
{
    public class WeaponTester
    {
        ...
        //            Console.WriteLine();
        //        }
        private void UseWeapon(Weapon type, int noOfUses)
        {
            Console.WriteLine($"Testing Wand {type.Description}");
            
            for (int i = 0; i < noOfUses; i++)
            {
                Console.WriteLine($"Damage dealt by {type.Description}: {type.DealDamage()}");
            }
            Console.WriteLine();
        }
